...
void Search(BTreeNode *Node, int Key) {
    int idx=0;
    while (idx<Node->KeyIndex && Node->Keys[idx]<Key) {
        idx++;
    }

    //if Key is found in Node
    if (idx<Node->KeyIndex && Node->Keys[idx]==Key) {
        printf("Key %d exist in tree\n", Key);
    }

    //if Key is not found in Node and Node is Leaf
    else if (Node->Leaf==TRUE) {
        printf("Key %d not exist in tree\n", Key);
    }

    //if Key is not found in Node but Node is not Leaf
    else {
        if (Node->Keys[idx-1]>Key) {
            Search(Node->Childs[idx-1], Key);
        }
        else {
            Search(Node->Childs[idx], Key);
        }
    }
}
...
void BorrowFromNext(BTreeNode *Node, int Index) {
    ...
    //keys[index] is inserted as the last key in childs[index]
    Child->Keys[Child->KeyIndex]=Node->Keys[Index];

    //first child of sibling is inserted as the last child into childs[index]
    if (Child->Leaf==FALSE) {
...
void Merge(BTreeNode *Node, int Index) {
    ...
    //moving key from the Node and insert it into NODE_DEGREE-1 th location of childs[index]
    ...
    //moving all keys after index in the Node one step before to fill the space created by moving keys
    for (int i=Index+1;i<Node->KeyIndex;i++) {
        Node->Keys[i-1]=Node->Keys[i];
    }
